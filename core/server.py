    ...
    def train(self):
        ...
        try:
            ...
            for i in range(self.cur_iter_no, self.max_iteration):
                ...
                to_add = [float(self.metrics['best_val_loss']),float(self.metrics['best_val_acc']),float(self.metrics['best_test_loss']),float(self.metrics['best_test_acc'])]
                main_dataframe.loc[i] = to_add
                print(i, to_add)
                print(main_dataframe.head())
                print(type(self.worker_trainer.model))
